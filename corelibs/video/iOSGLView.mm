...
-(void)setBufferYUV2:(const uint8_t*)buffer andWidth:(uint)bufferWidth andHeight:(uint)bufferHeight {
 
    if (!_animating)
        _animating = GL_TRUE;
    
    BOOL lock = [self.renderLock tryLock];
    if (lock == NO) {
        NSLog(@"render pass.");
        return;
    }
    
    [self setBufferYUV:buffer andWidth:bufferWidth andHeight:bufferHeight];
...
- (void)render:(CADisplayLink*)displayLink {
    ...
    if(_buffer && _bufferSize){
        ...
        glVertexAttribPointer(ATTRIBUTE_VERTEX, 2, GL_FLOAT, 0, 0, _position);
...
- (void)updateVertices {
    ...
    const float dH      = (float)_bufferHeight / height;
    const float dW      = (float)_bufferWidth	  / width;
    const float dd      = fit ? MIN(dH, dW) : MAX(dH, dW);
    float h             = (height * dd) / _bufferHeight;
    float w             = (width  * dd) / _bufferWidth;
